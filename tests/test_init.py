...
async def test_coordinator_logs_diagnostic_for_meters_missing_a_reading(
    ...
    # caplog normally relies on records propagating up to the root logger,
    # but pytest-homeassistant-custom-component's `hass` fixture installs
    # its own logging setup (queue handlers, propagate tweaks) that can
    # keep root-level capture from seeing our records. Attaching caplog's
    # handler straight to our named logger sidesteps that entirely.
